 ...
 *
 * Usage:
 *   node scripts/verify-broken-patterns.mjs [scanPath]
...
async function verifyBrokenPatterns(scanPath) {
  const results = [];
  let totalHits = 0;

  for (const pattern of BROKEN_PATTERNS) {
    // Escape special chars for use in grep
    ...
    // -E enables extended regex for word boundaries
